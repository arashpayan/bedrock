...
### Participation
Community participation in contributing to the Fund, per Bahá'í month. Added in `migrations/0007_participation.sql` (schema v7); methods in `participation_methods.go`.
- `Participation` - `AdultsContributed`/`AdultsActive`, plus `Youth`, `JuniorYouth`, `Children`. Only adults have a denominator, so only adults yield a rate: `AdultRate()` (0–100), `HasAdultRate()`, `Contributors()`.
- **Methods:** `SetParticipation(period, ParticipationCounts)` (upsert), `Participation(period)` (nil,nil when unrecorded), `ParticipationForPeriods([]BadiPeriod)`, `DeleteParticipation(period)`.
- **These figures cannot be derived from receipts** — a contribution may come from a household, arrive anonymously, or be given outside the Assembly's records — so every month is optional and gathered by hand. **A missing month means "not known", never "nobody contributed";** renderers must keep the two distinct.
- **Ayyám-i-Há is not a valid key.** Those days are counted with **Mulk** (month 18), the month they follow, so a Mulk record covers 23–24 days. `SetParticipation`/`Participation`/`DeleteParticipation` reject it, and a `MonthlyReport` for Ayyám-i-Há returns `Participation == nil` with `ParticipationBelongsTo` naming Mulk.
- **Never `ORDER BY badi_month`.** Ayyám-i-Há is month 0 but falls *between* months 18 and 19, so ordering or range-scanning on that column silently misplaces the tail of every year. `ParticipationForPeriods` takes an already-ordered period list (from `BadiPeriodsInRange`) and does point lookups, returning a slice **aligned by index** with nil for unrecorded months.
- Validation: nothing negative, and `AdultsContributed <= AdultsActive` — enforced both in Go and by table CHECK constraints.
...
- **Participation:** `Participation` (nil when unrecorded), `ParticipationBelongsTo` (set only on an Ayyám-i-Há report), and `ParticipationHistory` — the **named** months of the fiscal year up to and including this period, oldest first, for charting the adult rate. Ayyám-i-Há never appears in the history. A fiscal year that opens before the Naw-Rúz table begins is **clamped** to the table's start rather than failing the report.
